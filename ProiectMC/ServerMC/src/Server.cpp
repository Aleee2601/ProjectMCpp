...
//
//       
//        auto players = m_gameSession->GetAllPlayers();  // Obținem toți jucătorii activi
//
...
//            playerInfo["lives"] = player.GetLives();  // Adaugă viețile
//            playerInfo["kills"] = player.GetKills();  // Adaugă inamicii doborâți
//            playersList.emplace_back(std::move(playerInfo));  // Adăugăm informațiile despre jucător
//        }
//
//        response["players"] = std::move(playersList);  // Adăugăm lista de jucători în răspuns
//        return crow::response(response);
//            });
//
//}
